...
void main() {
  ...
  vAlpha = 1.0; // No fading needed for rectangles
}
`;
...
class RectangleGeometry extends Geometry {
  constructor(width = 1.0, height = 0.5625) {
    // 16:9 aspect ratio (height = 9/16 of width)
    super();
    const halfWidth = width / 2;
    const halfHeight = height / 2;

    // Define 4 vertices for a rectangle (16:9)
    this.addVertex(
      -halfWidth,
      -halfHeight,
      0, // Bottom-left
    ...
    // Define UVs for texture mapping (unchanged)
    this.vertices[0].uv[0] = 0;
    this.vertices[0].uv[1] = 0; // Bottom-left
    this.vertices[1].uv[0] = 1;
    this.vertices[1].uv[1] = 0; // Bottom-right
    this.vertices[2].uv[0] = 1;
    this.vertices[2].uv[1] = 1; // Top-right
    this.vertices[3].uv[0] = 0;
    this.vertices[3].uv[1] = 1; // Top-left

    // Define 2 triangles to form the rectangle
    this.addFace(0, 1, 2); // First triangle
    this.addFace(0, 2, 3); // Second triangle
  }
}
...
this.rectGeo = new RectangleGeometry(1.0, 0.5625); // 16:9 aspect ratio
    this.discBuffers = this.rectGeo.data;
  ...
  return (
    <div className="relative w-full sm:h-full h-2/3">
      <canvas
        ...
        ref={canvasRef}
        className="cursor-grab w-full h-full overflow-hidden object-cover relative outline-none active:cursor-grabbing"
      />
      ...
      {activeItem && (
        ...
        `}
          >
            <p className="flex items-center gap-3 select-none relative text-[#060606] top-[2px] text-[26px]">
